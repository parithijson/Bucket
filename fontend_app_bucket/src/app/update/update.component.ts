...
ngOnInit() {
  ...
  constructor(private formBuilder: FormBuilder,private SharedService: SharedServiceService,private upload:UploadService,private postproj:ProjectService,private route:ActivatedRoute,private router:Router) {
...
const mediaObjects: { mediaURL: string }[] = this.mediaURLs.map(mediaURL => {
  return { mediaURL: mediaURL };
    ...
    this.project = {'projectTitle':this.formData.get('projectTitle'),'projectDescription':this.formData.get('projectDescription'),"media":mediaObjects,'tags':tagsArray};
console.log(this.project);

    this.postproj.updateProject(this.projectID,this.project).subscribe((data)=>{
      console.log(data);
